...
TradesDF = pd.DataFrame(columns=['Agent#','Time Period', 'TradeID', 'Quantity (kWh)', 'Price (c/kWh)'])
...
#need to retrieve trade bids from blockchain
p = part[num_agent-1].nonzero()[0]
print('Agent', num_agent, 'Step 2')
#now, each agent needs to collect all trade bids from other agents and repeat the process
sender = str(web3.eth.accounts[num_agent])
peer = contract.functions.peers(sender).call()
peer_id = peer[0]
p = part[num_agent-1].nonzero()[0]
for j in range(len(p)):
    ...
    pos = p[j]
    pos = pos.tolist()
    Trades[pos] = trade_temp[2]/10000

print('Agent', num_agent, 'Trades', Trades)
...
p = part[num_agent-1].nonzero()[0]
for j in range(len(p)):
    ind_row_current = TradesDF.shape[0]
    TradesDF.loc[ind_row_current, 'Agent#'] = "agent" + str(num_agent)
    TradesDF.loc[ind_row_current, 'Time Period'] = time_period
    TradesDF.loc[ind_row_current, 'TradeID'] = str(num_agent) + str(p[j]+1)
    TradesDF.loc[ind_row_current, 'Quantity (kWh)'] = temp[p[j].tolist()]
    TradesDF.loc[ind_row_current, 'Price (c/kWh)'] = prices[j]
    ind_row_current = ind_row_current + 1

TradesDF.to_csv(r"C:\Users\Inessa\Desktop\4th_Year\FYP\Methodology_Eth\combination\testing_parallel4\Results\Trades1.csv")
