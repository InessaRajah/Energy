...
num_agents = contract.functions.peersCount().call()
...
agent, agents = func.createAgent(loads[num_agent-1][time_period], SOC_bat[num_agent-1][0], solar_panels[num_agent-1], agents, solar, time_period, k_tim)
...
while (optimal == 0):
   ...
   if (optimal == 0):
      ...
      #wait until everyone has submitted trade bids
      while(num_tradebids<((num_agents)*(num_agents-1))):
         num_tradebids = contract.functions.tradeCountIter().call()
      ...
      for j in range(len(p)):
         ...
         pos = p[j]
         pos = pos.tolist()
         Trades[pos] = trade_temp[4]/10000
      print('Trades to 1', Trades)

print('OPTIMAL TRADES FOUND')
time_stop = TIME.time()
print('TIME TAKEN', time_stop - start_time)